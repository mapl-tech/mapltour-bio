...
import { guide, followup1, followup2 } from './emails.mts'

/**
 * POST /api/lead { email, website?, source?, page? }
...
export default async (req: Request, _ctx: Context) => {
  ...
  if (body.website && body.website.trim()) return json(200, { ok: true })

  const email = (body.email ?? '').trim().toLowerCase().slice(0, 200)
  if (!EMAIL.test(email)) return json(400, { error: 'Please enter a valid email address.' })
