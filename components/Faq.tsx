...
const QA = [
  ...
  { q: 'How many people fit in one fare?', a: 'Up to 4 passengers with normal luggage for the flat fare. Five to seven are priced in the finder above; eight or more are quoted by email.' },
        ...
        <div className="faq">
          {QA.map((x, i) => (
            <details key={x.q} open={i === 0}>
              <summary>{x.q}</summary>
              <div className="faq-a">{x.a}{i === 2 ? <> <a className="inline-link" href={out('/terms', 'faq_terms')}>Read the terms</a>.</> : null}</div>
            </details>
          ))}
        </div>
